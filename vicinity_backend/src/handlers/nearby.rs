...
use crate::{error::AppError, match_engine::{scoring, vector}, state:: AppState};
...
pub async fn get_nearby(State(state): State<AppState>, Query(query): Query<NearbyQuery>)-> Result<Json<NearbyResponse>, AppError>{
    ...
    let members = state.redis.zrangebyscore_venue_users(&venue_id, cutoff).await?;
    ...
    let Ok(other_id) = Uuid::parse_str(&member_id_str) else{continue;};
    if other_id == me {
        continue;
    }
    ...
    if crate::repos::blocks::is_blocked(&state.db, me, other_id).await? { continue; }
    ...
    // Compute score.
        ...
        let distance_label = if age <= 10 { "here" }
            else if age <= 60 { "close" }
            else { "near" };
  ...
  // 4. Sort by match score descending.
    out.sort_by(|a, b| b.match_score.partial_cmp(&a.match_score).unwrap_or(std::cmp::Ordering::Equal));
