...
const NEARBY: Person[] = [
  {
    ...
    artist: "The Neighbourhood, Arctic Monkeys",
    movie: "Interstellar, Euphoria",
    spots: "Columbia Library, Back Bar",
    smoker: false,
    drinker: true,
    direction: 45,
    picsUnlocked: false,
  },
  {
    ...
    playlist: "Blinding Lights — The Weeknd",
    artist: "The Weeknd, Doja Cat",
    movie: "La La Land, Sex Education",
    spots: "Soho House, Eataly",
    smoker: false,
    drinker: true,
    direction: 120,
    picsUnlocked: false,
  },
  {
    ...
    playlist: "Solar Power — Lorde",
    artist: "Lorde, Clairo, Frank Ocean",
    movie: "Moonlight, Call Me By Your Name",
    spots: "Washington Square, McNally Jackson",
    smoker: false,
    drinker: false,
    direction: 200,
    picsUnlocked: false,
  },
  {
    ...
    artist: "Sabrina Carpenter, Olivia Rodrigo",
    movie: "Portrait of a Lady on Fire, Amélie",
    spots: "Chelsea Gallery, The Highline",
    smoker: false,
    drinker: true,
    direction: 300,
    picsUnlocked: false,
...
const getProfilePic = (id: string) => {
  switch (id) {
    case "1":
      return "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=500";
    case "2":
    ...
    case "4":
      return "https://images.unsplash.com/photo-1517841905240-472988babdf9?w=500";
    default:
        ...
        {isMatched ? (
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 2 }}
            />
            <Text style={[c.lockText, { color: `${accent}CC` }]}>
              Revealed on match
            </Text>
          </View>
        )}
      ...
      <View style={c.info}>
        <View style={c.nameRow}>
          ...
          >
            ...
            <Text style={[c.distText, { color: accent }]}>
              ~{person.distance}m
            </Text>
          </View>
        </View>
        ...
        <View style={[c.quoteWrap, { borderLeftColor: `${accent}70` }]}>
          <Text style={c.quote} numberOfLines={2}>
            "{person.prompt}"
          </Text>
        </View>
        ...
        <View style={c.actions}>
          ...
          <View style={{ flex: 1, position: "relative" }}>
            ...
            <Animated.View
              style={{
                transform: [
                  { scale: Animated.multiply(waveScale, waveBtnScale) },
                ],
              }}
            >
              <Pressable
                ...
                style={[
                  ...
                  waved
                    ...
                    : {
                        backgroundColor: waveHovered
                          ? "rgba(255,255,255,0.07)"
                          : "rgba(255,255,255,0.04)",
                        borderColor: waveHovered
              ...
              >
                ...
                <Text
                  style={[
                    c.waveBtnText,
                    { color: waved ? accent : "rgba(255,255,255,0.5)" },
                  ]}
                >
                  {waved ? "Waved!" : "Wave"}
                </Text>
          ...
          >
            ...
            >
              ...
              <Animated.View
                style={{
                  transform: [{ translateX: viewArrowX }],
                  marginLeft: 6,
                }}
              >
      ...
      >
              ...
              style={[
                sh.knob,
                {
                  backgroundColor: settings.visible
                    ? accent
                    : "rgba(255,255,255,0.2)",
                  alignSelf: settings.visible ? "flex-end" : "flex-start",
                },
              ]}
            />
          </Pressable>
        ...
        <View style={sh.chips}>
          {RADII.map((r) => (
            <Pressable
              key={r}
              onPress={() => setSettings((p) => ({ ...p, radius: r }))}
              style={[
                sh.chip,
                settings.radius === r && {
                  backgroundColor: `${accent}20`,
        ...
        <View style={sh.chips}>
          {SHOW_OPTIONS.map((o) => (
            <Pressable
              key={o}
              onPress={() => setSettings((p) => ({ ...p, showMe: o }))}
              style={[
                sh.chip,
                settings.showMe === o && {
                  backgroundColor: `${accent}20`,
        ...
        <BouncyButton
          onTap={onClose}
          style={[sh.doneBtn, { backgroundColor: accent }]}
        >
...
function AmbientStrip({ count, accent }: AmbientStripProps) {
  ...
  const r1Op = ripple1.interpolate({
  ...
  const r2Op = ripple2.interpolate({
    inputRange: [0, 0.2, 0.8, 1],
    outputRange: [0, 0.45, 0.15, 0],
  });
  ...
  return (
    <Animated.View style={[a.strip, { opacity: op }]}>
      ...
      >
        <Text style={[a.countNum, { color: accent, fontFamily: F.serif }]}>
          {count}
        </Text>
      </Animated.View>
      <Text style={[a.label, { fontFamily: F.regular }]}>
        people nearby right now
      </Text>

      <View style={a.dotsRow}>
        {Array.from({ length: count }).map((_, i) => {
          const bobY = bobAnims[i % bobAnims.length].interpolate({
            inputRange: [0, 1],
            outputRange: [-2.5, 2.5],
          });
          return (
            <Animated.View
              key={i}
              style={{ transform: [{ translateY: bobY }] }}
            >
              <View style={[a.dot, { backgroundColor: accent }]} />
            </Animated.View>
          );
        })}
      </View>
...
function WaveToast({ visible, name }: WaveToastProps) {
  ...
  return (
    <Animated.View
      style={[t.toast, { opacity: op, transform: [{ translateY: ty }] }]}
    >
...
export default function RadarScreen({
  ...
  const load = useCallback(async () => {
    try {
      const list = await getNearby(userId);
      // Map backend shape → Person shape the UI expects.
      const mapped: Person[] = list.map((u: NearbyUserBackend) => ({
        id: u.id,
        name: u.display_name,
        age: "--", // backend doesn't return age yet
        college: "", // nor college
        distance:
          u.distance_label === "here"
            ? 10
            : u.distance_label === "close"
              ? 50
              : 150,
        matchPercent: Math.round(u.match_score),
        favColor: accent,
        mood: u.interest_tags.slice(0, 2).join(" · "),
        vibes: u.interest_tags.slice(0, 3),
        prompt: "Say hi if you vibe.",
        playlist: "",
        artist: "",
        movie: "",
        spots: "",
  ...
  useEffect(() => {
    load();
    // Refresh every 30 seconds while the screen is open.
