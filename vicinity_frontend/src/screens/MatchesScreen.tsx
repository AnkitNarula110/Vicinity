      ...
      <Pressable onPress={handlePress} style={s.row}>
        ...
        <View style={s.info}>
          ...
          <View style={s.distRow}>
            ...
            <Text style={[s.dist, { color: accent }]}>
              {" "}
  ...
  return (
    <View style={es.wrap}>
      <Animated.View
        style={[
          es.ring,
          { borderColor: `${accent}30`, transform: [{ scale: pulse }] },
        ]}
      />
...
export default function MatchesScreen({
  ...
  const load = useCallback(async () => {
    try {
      const nudges = await getMatches(userId);
      // Map Nudge → Match (the shape the UI expects).
      const mapped: Match[] = nudges
        .filter((n) => n.status === "mutual" || n.status === "pending")
        .map((n) => ({
          id: n.id,
          name: "Match", // backend doesn't return name yet
          age: "--",
          distance: 30,
          favColor: accent,
          lastMsg:
            n.status === "mutual"
              ? "You both nudged — say hi!"
              : "Nudge sent — waiting for them",
          time: new Date(n.updated_at).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
          unread: 0,
        ...
        >
          {MOCK_MATCHES.map((m) => (
            <MatchRow
              key={m.id}
              match={m}
              onPress={(match) => onOpenChat(match)}
            />
          ))}
